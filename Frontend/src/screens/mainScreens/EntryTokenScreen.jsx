...
const EntryTokenScreen = ({ navigation }) => {
  ...
  const validateInputs = () => {
    if (entryToken.length !== 4) {
      setError('Please enter a valid 4-digit token');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setModalMessage(error.response?.data?.message || 'Invalid entry token');
  ...
  return (
    <View style={{ flex: 1 }}>
      <View style={styles.header}>
        <Icon name="key" size={28} color="#4CAF50" />
        <Text style={styles.title}>Entry Token</Text>
      </View>
      <View style={styles.container}>
        <View style={styles.card}>
          <Text style={styles.instructionText}>Enter your 4-digit entry token</Text>
      ...
      <Modal visible={!!booking} transparent animationType="fade">
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalText}>Entry time has been successfully recorded.</Text>
            <Text style={styles.modalToken}>Token: {booking?.entryToken}</Text>
            <TouchableOpacity
...
export default EntryTokenScreen;